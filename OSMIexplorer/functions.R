...
charting_functions[["scatterplot"]] <- function(x, y, group = "None") {
  ...
  # build ggplot
  if (group != "None") {
    gg <- ggplot(df, aes(x = !!sym(x), y = !!sym(y), color = !!sym(group))) +
      geom_point() +
      geom_smooth()
  } else {
...
charting_functions[["bar"]] <- function(x, y = "None", group = "None") {
  ...
  if (group != "None" & y != "None") {
    gg <- ggplot(df) +
      stat_summary(aes(x = !!sym(x), y = !!sym(y), fill = !!sym(group)), fun = "mean", geom = "bar", position = position_dodge()) +
      ylab(paste0("Mean: ", y)) +
      coord_flip()
  } else if (y != "None") {
    gg <- ggplot(df) +
      stat_summary(aes(x = !!sym(x), y = !!sym(y)), fun = "mean", geom = "bar") +
      ylab(paste0("Mean: ", y)) +
      coord_flip()
  } else if (group != "None") {
    gg <- ggplot(df) +
      geom_bar(aes(x = !!sym(x), fill = !!sym(group))) +
      coord_flip()
  } else {
    gg <- ggplot(df) +
...
charting_functions[["us_map"]] <- function(x, y = "None") {
  ...
  if (y != "None") {
    ...
    leaflet(states_it) %>%
      ...
      addPolygons(
        fillColor = ~ mypalette(states_it@data[[y]]), stroke = FALSE,
        label = mytext,
    ...
    leaflet(states_it) %>%
      ...
      setView(lat = 39.8333333, lng = -98.585522, zoom = 3) %>%
      addPolygons(
        fillColor = ~ mypalette(states_it@data$count), stroke = FALSE,
        label = mytext,
        labelOptions = labelOptions(
          style = list("font-weight" = "normal", padding = "3px 8px"),
          textsize = "13px",
...
charting_functions[["world_map"]] <- function(x, y = "None") {
  ...
  if (y != "None") {
    ...
    leaflet(world_it) %>%
      addTiles() %>%
      addPolygons(
        fillColor = ~ mypalette(world_it@data[[y]]), stroke = FALSE,
        label = mytext,
        labelOptions = labelOptions(
          style = list("font-weight" = "normal", padding = "3px 8px"),
          textsize = "13px",
    ...
    leaflet(world_it) %>%
      addTiles() %>%
      addPolygons(
        fillColor = ~ mypalette(world_it@data$count), stroke = FALSE,
        label = mytext,
        labelOptions = labelOptions(
          style = list("font-weight" = "normal", padding = "3px 8px"),
          textsize = "13px",
